...
## Objects::create
    ...
    Objects::create($structure, $fill = null)

Функция создает массив данных по заданной структуре с указанным заполнением

**structure**
Тип: **смешанный**
Заданная структура массива

**fill**
Тип: **смешанный** *(необязательный)*
Массив заполнения

Функция возвращает новый массив, созданный по заданной структуре.

Поддерживаются многоуровневые вложения.
...
Пример:
    ...
    $result = Objects::create($structure, $fill);
...
Структура может быть передана в виде json строки.
...
## Objects::createByIndex

    Objects::createByIndex($structure, $fill = null)

Функция создает массив данных по заданной структуре индексов с указанным заполнением
...
Работа аналогичка функции **Objects::create**, но теперь структура распознается только как массив индексов без поддержки рекурсии в индексах.

Пример:

    $structure = ['a', 'b', 'c'];
    $fill = ['a' => 1];
    $result = Objects::createByIndex($structure, $fill);
...
Структура также может быть передана в виде json строки или строки в системном формате данных.
...
Например:
    ...
    Objects::eachOf($array, [ 'str' => null ], function ($v, $k, $p){ $p['str'] .= '...'; });
...
## Objects::recurse
    ...
    Objects::recurse(&$item, $callback, $ignore = null)

Функция аналогична **Objects::each**, но работает рекурсивно.

> Отсутствует третий аргумент в callback-функции с указанием позиции текущего элемента.
...
## Objects::sort
    ...
    Objects::sort($haystack, $keys = false, $associate = 'default')
...
Третьим аргументом можно принудительно включить или выключить ассоциированный тип массива. По-умолчанию, тип массива определяется автоматически.
